...
var TicTacToeGui = function() {
	...
	function mark(cell,symbol) { //put a symbol in one cell
		if (!cell) return false;
		cell.classList.remove("blank");
		return (cell.innerHTML = symbol);
	}

	function unmark(cell) { // remove  (and return) symbol from cell, if any
		if (!cell) return false;
	...
	function prepareCell(board,cell,x,y) {
		...
		cell.addEventListener('mouseover',function() {
			...
			if (typeof symbol !== 'string')
				symbol = '';
			cell.setAttribute('symbol',symbol); // special attribute for CSS to use in creating :hover content
		});
	}
	...
	var boardID = 0; //will increment with each new board
